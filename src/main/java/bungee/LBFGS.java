...
public class LBFGS implements Serializable {
        ...
        public void lbfgs(int n, int m, double[] x, double f, double[] g,
                ...
                if (iflag[0] == 0) {
                        ...
                        for (i = 0; i < n; i += 1) {
                                w[ispt + i] = -g[i] * diag[i];
                                //if (badNum(w[ispt + i]))
                                //        System.err.println("w1 " + g[i] + " " + diag[i]);
                        }
                ...
                while (true) {
                        ...
                        if (execute_entire_while_loop || iflag[0] == 2) {
                                if (iter != 1) {
                                        ...
                                        for (i = 0; i < bound; i += 1) {
                                                ...
                                                beta = w[inmc] - beta;
                                                if (badNum(beta)) {
                                                        throw new IllegalArgumentException(w[inmc]+" "+safeMult(w[n + cp] , yr));
                                                }
                                                iscn = ispt + cp * n;
                                        ...
                                        for (i = 0; i < n; i += 1) {
                                                ...
                                                //if (badNum(w[ispt + point * n + i]))
                                                //        System.err.println("w5 " + w[i]);
                                        }
                                }

                                nfev[0] = 0;
                                linesearch.setSTP(stp, iter == 1 && stp1 < 1 ? stp1 : 1);

                                for (i = 0; i < n; i += 1) {
                                        w[i] = g[i];
                                        //       if (badNum(w[i]))
                                        //               System.err.println("w6 " + g[i]);
                                }
                        }
                        ...
                        linesearch.mcsrch(this,n, x, f, g, w, ispt + point * n, stp, ftol, xtol,
                        ...
                        if (iprint[0] > 0)
                                System.err.println("msrch return = nfev=" + nfev[0] + " nfun="
                                        + nfun + " info=" + info[0] + " bestx=" + linesearch.stx[0]
                                        + " farx=" + linesearch.sty[0] + " brackt="
                                        + linesearch.brackt[0] + " stp=" + stp[0] + " gnorm="
                        ...
                        for (i = 0; i < n; i += 1) {
                                w[ispt + npt + i] *= stp[0];
                                //if (badNum(w[ispt + npt + i]))
                                //        System.err.println("w6 " + stp[0]);
                                w[iypt + npt + i] = g[i] - w[i];
                                //if (badNum(w[iypt + npt + i]))
                                //       System.err.println("w7 " + g[i] + " " + w[i]);
                        }
        ...
        public void lb1(int[] iprint, int iter1, int nfun1, double gnorm1,
                ...
                if (iter1 == 0) {
                        ...
                        if ((iprint[0] == 0) && (iter1 != 1 && !finish1))
                                return;
                        if (iprint[0] != 0) {
                                if ((iter1 - 1) % iprint[0] == 0 || finish1) {
