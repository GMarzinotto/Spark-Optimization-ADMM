...
public class Mcsrch implements Serializable{
        ...
        public void mcsrch(LBFGS thislbfgs, final int n, double[] x, double f, double[] g,
...
//              System.err.println("mc " + f + " " + stp[0]);
                ...
                if (info[0] != -1) {
                        infoc[0] = 1;
                        if (n <= 0 || stp[0] <= 0 || ftol < 0 || thislbfgs.gtol < 0 || xtol < 0
                                || thislbfgs.stpmin < 0 || thislbfgs.stpmax < thislbfgs.stpmin
                                || maxfev <= 0)
                                return;
                        ...
                        finit = f;
                        dgtest = ftol * dginit;
                        width = thislbfgs.stpmax - thislbfgs.stpmin;
                ...
                while (true) {
                        if (info[0] != -1) {
                                ...
                                setSTP(stp, Math.max(stp[0], thislbfgs.stpmin));
                                setSTP(stp, Math.min(stp[0], thislbfgs.stpmax));
                        ...
                        if (stp[0] == thislbfgs.stpmax && f <= ftest1 && dg <= dgtest)
                                // The step is at the upper bound <code>stpmax</code>.
                                info[0] = 5;

                        if (stp[0] == thislbfgs.stpmin && (f > ftest1 || dg >= dgtest))
                        ...
                        if (f <= ftest1 && Math.abs(dg) <= thislbfgs.gtol * (-dginit))
                        ...
                        if (stage1 && f <= ftest1
                                && dg >= Math.min(ftol, thislbfgs.gtol) * dginit)
                                stage1 = false;
