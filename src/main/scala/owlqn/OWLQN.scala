 ...
 * @author dlwh
 */
class OWLQN2[K, T](maxIter: Int, m: Int, l1reg: K => Double, tolerance: Double, Tinicial: Double)(implicit space: MutableEnumeratedCoordinateField[T, K, Double]) extends LBFGS[T](maxIter, m, tolerance=tolerance) with SerializableLogging {

  var store: List[Tuple3[Int,Double,T]] = Nil
  val startTime = Tinicial

  def this(maxIter: Int, m: Int, l1reg: K => Double)(implicit space: MutableEnumeratedCoordinateField[T, K, Double]) = {
    this(maxIter, m, l1reg, 1E-8,System.nanoTime()/1000.0)
  }

  def this(maxIter: Int, m: Int, l1reg: Double, tolerance: Double = 1E-8)(implicit space:MutableEnumeratedCoordinateField[T, K, Double]) = this(maxIter, m, (_: K) => l1reg, tolerance,System.nanoTime()/1000.0)

  def this(maxIter: Int, m: Int, l1reg: Double)(implicit space: MutableEnumeratedCoordinateField[T, K, Double]) = this(maxIter, m, (_: K) => l1reg, 1E-8,System.nanoTime()/1000.0)

  def this(maxIter: Int, m: Int)(implicit space: MutableEnumeratedCoordinateField[T, K, Double]) = this(maxIter, m, (_: K) => 1.0, 1E-8,System.nanoTime()/1000.0)
  ...
  override protected def determineStepSize(state: State, f: DiffFunction[T], dir: T) = {
    ...
    val ff = new DiffFunction[Double] {
      def calculate(alpha: Double) = {
        ...
        //Added to measure time differential
  ...
  override protected def takeStep(state: State, dir: T, stepSize: Double) = {
    ...
    val subresult = space.zipMapValues.map(stepped, orthant, { case (v, ov) =>
      v * I(math.signum(v) == math.signum(ov))
    })
    store :::= List((store.length + 1,(System.nanoTime()/1000.0 - startTime), state.x))
    subresult
  }
